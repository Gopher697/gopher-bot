...
func _ready() -> void:
    ...
    var err = _websocket.connect_to_url("ws://localhost:5000/socket.io/?EIO=4&transport=websocket")
    if err != OK:
...
func _process(_delta: float) -> void:
    ...
    if state == WebSocketPeer.STATE_OPEN:
        ...
        while _websocket.get_available_packet_count() > 0:
            ...
            _handle_socketio_packet(packet)
            
    elif state == WebSocketPeer.STATE_CLOSED:
        _connected = false

func _handle_socketio_packet(packet: String) -> void:
    if packet.begins_with("0"):
        # We connected to EIO, now connect to the /persona namespace
        _websocket.put_packet("40/persona,".to_utf8_buffer())
    elif packet.begins_with("42/persona,"):
        var json_str = packet.substr(11) # after "42/persona,"
        var json = JSON.new()
        if json.parse(json_str) == OK:
            var data = json.data
            if data is Array and data.size() > 1 and data[0] == "state_update":
                var payload = data[1]
                _update_avatar(payload)
